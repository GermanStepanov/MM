...
date: "`r format(Sys.Date(), '%d  %B  %Y')`"me
...
horsepower - Время разгона от 0 до 60 миль в час (сек.)
...
Построим графики разброса, показав факторы *origin* (число цилиндров)  цветом. Зависимой переменной модели является *mpg*, её покажем в первой строке / столбце матричного графика. Во вторую строку / столбец поставим фактор.


```{r}
# переводим дискретные количественные переменные в факторы
DF.auto$origin <- as.factor(DF.auto$origin)
# графики разброса, цвет -- количество цилиндров
ggpairs(DF.auto[, c(1, 2, 5)], ggplot2::aes(color = origin))
ggpairs(DF.auto[, c(1, 3, 5)], ggplot2::aes(color = origin))
ggpairs(DF.auto[, c(1, 4, 5)], ggplot2::aes(color = origin))
```


#### Только mpg ~ weight,  mpg ~ displacement,  mpg ~ horsepower
```{r}
...
$$mpg=f(weight + displacement + horsepower + origin)$$
Линейная модель: 

$$mpg=β_0+β_1⋅weihgt +β_2 displacement +β_3 horsepower+ origin$$
...
fit.lm.1_1 <- lm(mpg ~ weight + displacement + horsepower + origin, subset = inTrain)
# подгонка линейной модели на обучающей выборке
fit.lm.1_1 <- lm(mpg ~ weight + displacement + horsepower + origin, 
               subset = inTrain)
# прогноз на тестовую
...
$$mpg = β_0 + β_1weight + β_2 displacement + β_3 horsepower + β_4weight^2 + β_5 displacement^2 + β_6 horsepower^2 + β_7 origin + β_8 sylinders^2$$
...
attach(DF.auto)
# подгонка модели на обучающей выборке
fit.lm.2_1 <- lm(mpg ~ poly(weight, 2) + poly(displacement, 2) + poly(horsepower, 2) + origin, subset = inTrain)
# прогноз на тестовую
...
$$mpg=β_0+β_1weight+β_1weight + β_2 displacement + β_3 horsepower+ β_5 displacement^2 + 

+β_6 horsepower^2+ + β_4weight^2  β_4weight^2 + β_5 displacement^2 + β_6 horsepower^2+β_7⋅weight^3+ β_8 displacement^3 + β_9 horsepower^3 + β_10 sylinders + β_11 sylinders^2 + β_12 sylinders^3$$
...
attach(DF.auto)
# подгонка модели на обучающей выборке
fit.lm.3_1 <- lm(mpg ~ poly(weight, 3)  + poly(displacement, 3) + poly(horsepower, 3) + origin, 
               subset = inTrain)
# прогноз на тестовую
...
```{r}
# подгонка линейной модели на обучающей выборке
fit.glm_1 <- glm(mpg ~ weight + displacement + horsepower + origin, data = DF.auto)
...
for (i in 1:5) {
  # оценка модели
  fit.glm_1 <- glm(mpg ~ poly(weight, i)  + poly(displacement, i) + poly(horsepower, i) + origin, data = DF.auto)
  # расчёт ошибки
...
$$mpg=f(weight + displacement + horsepower)$$
Линейная модель: 

$$mpg=β_0+β_1⋅weihgt +β_2 displacement +β_3 horsepower$$
...
attach(DF.auto)
# подгонка модели на обучающей выборке
fit.lm.1 <- lm(mpg ~ weight + displacement + horsepower, subset = inTrain)
# подгонка линейной модели на обучающей выборке
...
$$mpg = β_0 + β_1weight + β_2 displacement + β_3 horsepower + β_4weight^2 + β_5 displacement^2 + β_6 horsepower^2$$
...
attach(DF.auto)
# подгонка модели на обучающей выборке
fit.lm.2 <- lm(mpg ~ poly(weight, 2) + poly(displacement, 2) + poly(horsepower, 2), subset = inTrain)
# прогноз на тестовую
...
+β_6 horsepower^2+ + β_4weight^2  β_4weight^2 + β_5 displacement^2 + β_6 horsepower^2+β_7⋅weight^3+ β_8 displacement^3 + β_9 horsepower^3$$
...
attach(DF.auto)
# подгонка модели на обучающей выборке
fit.lm.3 <- lm(mpg ~ poly(weight, 3)  + poly(displacement, 3) + poly(horsepower, 3), 
               subset = inTrain)
# прогноз на тестовую
...
```{r}
# подгонка линейной модели на обучающей выборке
fit.glm <- glm(mpg ~ weight + displacement + horsepower, data = DF.auto)
# считаем LOOCV-ошибку
...
for (i in 1:5) {
  # оценка модели
  fit.glm <- glm(mpg ~ poly(weight, i)  + poly(displacement, i) + poly(horsepower, i), data = DF.auto)
  # расчёт ошибки
...
boot.fn.2 <- function(data, index){
  coef(lm(mpg ~ poly(weight, 2) + poly(displacement, 2) +  poly(horsepower, 2), data = data, subset = index))
}
...
Нелинейность в остатках полинома третьей степени остаётся, и бутстреп-ошибки параметров модели выше, 1чем аналогичные МНК-оценки. 
